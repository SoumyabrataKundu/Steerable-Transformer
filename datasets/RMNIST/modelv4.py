...
class Model(nn.Module):
    def __init__(self, n_radius, max_m) -> None:
        ...
        self.network = nn.Sequential(
            FintConv2DType1(1, 6, 5, n_radius, n_theta, max_m, padding="same", device = device), # 96 X 96
            CGNonLinearity2D(max_m, device = device),
            FintConv2DType1(6, 16, 5, n_radius, n_theta, max_m, padding="same", device = device), # 96 X 96
            SteerableBatchNorm2D(),
            FintAvgPool2D(4),

            SE2TransformerEncoder(16,4,max_m, n_layers = 4,device = device),   # 6 X 6

            FintConv2DType2(16, 32, 7, n_radius, n_theta, max_m, device = device), #  6 X  6
