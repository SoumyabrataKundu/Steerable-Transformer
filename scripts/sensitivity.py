...
############################################################################################################################
    ...
    n_sim = 5
    noise_levels = [0.0000001, 0.000001, 0.00001, 0.0001, 0.001, 0.01, 0.1, 1]
    #noise_levels = [0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1]
    accuracy_stat = torch.empty(len(noise_levels), 2)
    for i, noise in enumerate(noise_levels):
        accuracy = []
        for run in range(n_sim):
            logger.info(f"\n\n\nTesting Noise {noise} Run {run+1}:\n")
            accuracy.append(test_loop(noise))
        mean = np.mean(accuracy)
        variance = np.var(accuracy)
        accuracy_stat[i,0] = mean
        accuracy_stat[i,1] = variance
        print(f'\n\nTesting Accuracy for {noise} is {mean * 100:.2f} +- {np.sqrt(variance) * 100:.4f} %')
